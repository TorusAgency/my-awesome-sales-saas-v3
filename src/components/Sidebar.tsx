...
import { 
  ...
  Columns // Added for Opportunities (Kanban icon)
} from 'lucide-react';
...
const navigationHierarchy = [
  ...
  { 
   ...
   { name: 'Opportunities', icon: Columns, view: 'opportunities' }, // Added Opportunities
          ...
          <div className="flex-1 flex flex-col overflow-y-auto">
            <nav className="flex-1 px-2 py-4 space-y-1">
              {navigationHierarchy.map((item) => {
                ...
                const isActiveParent = hasSubItems && (currentView === item.view || (item.view && currentView.startsWith(item.view + '/')) || item.subItems.some(sub => currentView === sub.view));
